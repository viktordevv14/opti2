      ...
      <nav className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center gap-2 cursor-pointer group" onClick={() => { setActiveTab('dashboard'); setViewingPatientId(null); setShowPatientForm(false); setShowPrescriptionForm(false); }}>
              <div className="bg-gradient-to-tr from-blue-600 to-indigo-700 p-2 rounded-xl shadow-lg group-hover:scale-110 transition-transform duration-300">
                <Glasses className="text-white w-6 h-6" />
              </div>
              <span className="text-2xl font-black bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-800 tracking-tighter">OptiCare</span>
            </div>
            <div className="flex gap-1 sm:gap-2 bg-gray-100 p-1.5 rounded-2xl">
              <button
                onClick={() => { setActiveTab('dashboard'); setViewingPatientId(null); setShowPatientForm(false); setShowPrescriptionForm(false); }}
                className={`px-4 py-2 text-sm font-black rounded-xl transition-all duration-300 ${activeTab === 'dashboard' ? 'text-blue-600 bg-white shadow-sm scale-105' : 'text-gray-500 hover:text-gray-900 hover:bg-white/50'}`}
              >
                Dashboard
              </button>
              <button
                onClick={() => { setActiveTab('patients'); setViewingPatientId(null); setShowPatientForm(false); setShowPrescriptionForm(false); }}
                className={`px-4 py-2 text-sm font-black rounded-xl transition-all duration-300 ${activeTab === 'patients' && !viewingPatientId ? 'text-blue-600 bg-white shadow-sm scale-105' : 'text-gray-500 hover:text-gray-900 hover:bg-white/50'}`}
              >
                Patients
              </button>
              <button
                onClick={() => { setActiveTab('prescriptions'); setViewingPatientId(null); setShowPatientForm(false); setShowPrescriptionForm(false); }}
                className={`px-4 py-2 text-sm font-black rounded-xl transition-all duration-300 ${activeTab === 'prescriptions' ? 'text-blue-600 bg-white shadow-sm scale-105' : 'text-gray-500 hover:text-gray-900 hover:bg-white/50'}`}
              >
                Prescriptions
              </button>
            </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        {activeTab === 'dashboard' && !showPatientForm && !showPrescriptionForm && !viewingPatientId && (
          <div className="animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-950 rounded-[2.5rem] p-8 sm:p-12 mb-10 shadow-2xl">
              <div className="absolute top-0 right-0 -mt-20 -mr-20 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl"></div>
              <div className="absolute bottom-0 left-0 -mb-20 -ml-20 w-72 h-72 bg-indigo-500/10 rounded-full blur-3xl"></div>
              ...
              <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                ...
                <div className="flex gap-4">
                   <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-3xl text-center min-w-[100px]">
                      <p className="text-blue-300 text-[10px] font-black uppercase tracking-widest mb-1">Status</p>
                      <p className="text-white font-bold flex items-center justify-center gap-2">
                        <Activity size={14} className="text-green-400" /> Online
                      </p>
                   </div>
                   <div className="bg-white/10 backdrop-blur-md border border-white/20 p-4 rounded-3xl text-center min-w-[100px]">
                      <p className="text-blue-300 text-[10px] font-black uppercase tracking-widest mb-1">Date</p>
                      <p className="text-white font-bold">{new Date().toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</p>
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12">
              <div className="group bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-4xl font-black text-gray-900 mt-1">{patients.length}</p>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-green-500 bg-green-50 w-max px-2 py-1 rounded-lg">
              ...
              <div className="group bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-50/50 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-500"></div>
                <div className="relative z-10 flex flex-col gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Prescriptions</p>
                    <p className="text-4xl font-black text-gray-900 mt-1">{prescriptions.length}</p>
                  </div>
                  <div className="flex items-center gap-2 text-xs font-bold text-emerald-500 bg-emerald-50 w-max px-2 py-1 rounded-lg">
                    <Activity size={12} /> Active clinic
                  </div>
                </div>
              </div>

              <div className="group bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-purple-50/50 rounded-full -mr-16 -mt-16 group-hover:scale-150 transition-transform duration-500"></div>
                <div className="relative z-10 flex flex-col gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">New Intake</p>
                  ...
                  <div className="text-xs font-bold text-gray-400">Current Month Data</div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <button
                    ...
                    className="group bg-white p-8 rounded-[2.5rem] border border-gray-100 hover:border-blue-500 hover:shadow-[0_20px_50px_rgba(37,99,235,0.15)] transition-all duration-500 flex flex-col items-center text-center gap-6 relative overflow-hidden"
                  >
                    ...
                    <div className="bg-blue-50 p-6 rounded-[2rem] text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500 shadow-inner relative z-10">
                      <UserPlus size={32} />
                    </div>
                    <div className="relative z-10">
                      <span className="block font-black text-gray-900 text-lg">Add Patient</span>
                      <span className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1 block">New Record</span>
                    </div>
                  </button>
                  <button
                    onClick={() => { setShowPrescriptionForm(true); setActiveTab('prescriptions'); }}
                    className="group bg-white p-8 rounded-[2.5rem] border border-gray-100 hover:border-emerald-500 hover:shadow-[0_20px_50px_rgba(16,185,129,0.15)] transition-all duration-500 flex flex-col items-center text-center gap-6 relative overflow-hidden"
                  >
                    <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-50/50 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-700"></div>
                    <div className="bg-emerald-50 p-6 rounded-[2rem] text-emerald-600 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-500 shadow-inner relative z-10">
                      <FileText size={32} />
                    </div>
                    <div className="relative z-10">
                      <span className="block font-black text-gray-900 text-lg">Issue Rx</span>
                      <span className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1 block">Prescription</span>
                    </div>
                  </button>
                </div>

                <div className="bg-gradient-to-br from-indigo-600 to-blue-800 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
                  <div className="absolute top-0 right-0 -mt-10 -mr-10 w-64 h-64 bg-white/10 rounded-full blur-3xl group-hover:bg-white/20 transition-colors duration-700"></div>
                  <div className="relative z-10">
                    <div className="bg-white/20 backdrop-blur-md w-12 h-12 rounded-2xl flex items-center justify-center mb-6 border border-white/30">
                      <Users size={24} />
                    </div>
                    <h4 className="font-black text-3xl mb-3 tracking-tight text-white">Database Management</h4>
                    <p className="text-blue-100/80 mb-8 text-lg leading-relaxed">
                      Securely manage {patients.length} patient records with full clinical history.
                    </p>
                    <button
                      onClick={() => setActiveTab('patients')}
                      className="bg-white text-blue-700 px-8 py-4 rounded-2xl font-black transition-all hover:bg-blue-50 hover:shadow-xl hover:scale-105 active:scale-95 flex items-center gap-3"
                    >
                      Browse Database <ArrowLeft className="rotate-180" size={18} />
                    </button>
                  </div>
                  <Glasses className="absolute -right-10 -bottom-10 w-64 h-64 opacity-5 rotate-12 group-hover:rotate-45 transition-transform duration-1000" />
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between px-2">
                  <h3 className="text-2xl font-black text-gray-900">Recent Enrollments</h3>
                  <button onClick={() => setActiveTab('patients')} className="text-xs font-black text-blue-600 uppercase tracking-widest hover:underline">View All</button>
                </div>
                <div className="bg-white rounded-[2.5rem] border border-gray-100 divide-y divide-gray-50 shadow-sm overflow-hidden">
                  {[...patients].sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 5).map(p => (
                    <div key={p.id} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-all cursor-pointer group" onClick={() => { setViewingPatientId(p.id); setActiveTab('patients'); }}>
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl flex items-center justify-center text-blue-600 font-black text-xl shadow-inner border border-blue-100/50 group-hover:scale-110 transition-transform duration-300">
                          {p.name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-black text-gray-900 text-lg group-hover:text-blue-600 transition-colors">{p.name}</p>
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-xs text-gray-400 font-bold flex items-center gap-1">
                              <Calendar size={12} /> {new Date(p.createdAt).toLocaleDateString()}
                            </span>
                            <span className="w-1 h-1 bg-gray-200 rounded-full"></span>
                            <span className="text-xs text-gray-400 font-bold">{p.phone}</span>
                          </div>
                        </div>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-xl text-gray-400 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
                  ...
                  {patients.length === 0 && (
                    <div className="p-20 text-center flex flex-col items-center gap-4">
                      ...
                      <p className="text-slate-400 font-bold italic text-lg">Your practice is waiting for its first patient.</p>
                    </div>
                  )}
                </div>
              </div>
